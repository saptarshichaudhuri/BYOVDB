    ...
    def __init__(self, points, dim):
        ...
        def make(points, i=0):
            if len(points) > 1:
                ...
                #return structure [node_leftchild, node_rightchild, node itself]
                left_child = make(points[:m], i)
                right_child = make(points[m + 1:], i)
                pivot = points[m]
                arr = [left_child, right_child, pivot]
                return arr
        ...
        def add_point(node, point, i=0):
            if node is not None:
                ...
                for j, c in ((0, dx >= 0), (1, dx < 0)):
                    #if dx >= 0, then j points to 0 as in left child or else right child
